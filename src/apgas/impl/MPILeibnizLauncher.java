...
/** The {@link MPILeibnizLauncher} class implements a launcher using ssh. */
final class MPILeibnizLauncher implements Launcher {
  ...
  /** Constructs a new {@link MPILeibnizLauncher} instance. */
  MPILeibnizLauncher() {
    Runtime.getRuntime().addShutdownHook(new Thread(() -> terminate()));
  }

  @Override
  public void launch(int n, List<String> command, List<String> hosts, boolean verbose)
      throws Exception {
    ...
    String host;
    host = it == null ? InetAddress.getLoopbackAddress().getHostAddress() : it.next();
    ...
    for (int i = 0; i < n; i++) {
      ...
      if (it != null) {
        if (it.hasNext()) {
          ...
          System.err.println("[APGAS] Warning: hostfile too short; repeating last host: " + host);
          warningEmitted = true;
        }
      }
      Process process;
